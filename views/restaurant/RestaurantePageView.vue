<template>
  ...
  >
    ...
    <CommentsSection :restaurantId="restaurante.id" class="mt-12" />
  </div>
...
const carregarAvaliacoes = async (id) => {
  try {
    const res = await axios.get(`http://localhost:5001/api/review/restaurant/${id}`)
...
const averageRating = computed(() => {
  ...
  const total = reviews.value.reduce((sum, r) => sum + (r.avgRating || 0), 0)
...
const restaurantStatus = computed(() => {
  ...
  const openMinutes =
    parseInt(r.openTime.split(":")[0]) * 60 + parseInt(r.openTime.split(":")[1])
  const closeMinutes =
    parseInt(r.closeTime.split(":")[0]) * 60 + parseInt(r.closeTime.split(":")[1])
  ...
  if (!isOpenDay) return { text: "FECHADO", color: "text-red-600" }
