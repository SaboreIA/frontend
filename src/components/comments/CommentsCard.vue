<template>
  <div class="bg-white p-4 rounded-lg shadow-md border border-gray-100 mb-4">
      ...
      <div class="flex items-center text-gray-600">
        <div class="flex items-center mr-4">
          <span 
            v-for="n in 5" 
            :key="n" 
            :class="['w-3 h-3 rounded-full mr-1', n <= rating ? 'bg-yellow-600' : 'bg-gray-300']">
          </span>
        </div>
        
        <button 
          @click="$emit('toggleLike', commentId)" 
        ...
        >
          ...
          <span class="text-sm font-medium ml-1">{{ likes }}</span>
        </button>
    ...
    <div v-if="categories && categories.length" class="grid grid-cols-2 md:grid-cols-4 gap-y-3 mb-4">
      <div v-for="(cat, index) in categories" :key="index" class="flex flex-col items-start">
