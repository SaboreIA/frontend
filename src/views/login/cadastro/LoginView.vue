    ...
    <div class="w-full max-w-md p-8 rounded-lg space-y-8">
      <div class="text-center space-y-2">
        <div class="flex justify-center items-center space-x-2">
          ...
          <h2 class="text-3xl font-extrabold text-black">Entrar na Conta</h2>
        </div>
      </div>
      ...
      <form @submit.prevent="handleLogin" class="space-y-6">
        <div>
          ...
          <div
            class="mt-1 relative border border-gray-300 rounded-lg focus-within:border-amber-600 focus-within:ring-1 focus-within:ring-amber-600 transition duration-200"
          >
            <div
            ...
            >
              ...
              >
                <path
                  ...
                  stroke-linejoin="round"
                  stroke-width="2"
                  d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8m-1 10a2 2 0 01-2 2H6a2 2 0 01-2-2V7a2 2 0 012-2h12a2 2 0 012 2v11z"
                ></path>
              </svg>
            </div>
        ...
        <div>
          <router-link
            :to="{name: 'perfil-usuario', params: {username: 'john-doe'}}"
            custom
            v-slot="{navigate}"
          >
            <button
  ...
  methods: {
    async handleLogin() {
      ...
      if (this.email === MOCK_EMAIL && this.password === MOCK_PASSWORD) {
        // 1. Simula os dados que seriam retornados pelo backend
        ...
        alert(
          `Login bem-sucedido! Bem-vindo(a), ${simulatedUserData.name}! (Simulado)`
        );
        this.$router.push("/dashboard");
      } else {
        // Simula erro de credenciais
        alert(
          `Erro de Login: Credenciais inválidas (Simulado). Use "${MOCK_EMAIL}" e "${MOCK_PASSWORD}".`
        );
      }
      // === FIM DA SIMULAÇÃO ===
    },
